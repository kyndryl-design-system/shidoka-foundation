...
export class SplitButton extends LitElement {
   ...
   * @ignore
   */
  @queryAssignedElements({ selector: 'kd-splitbutton-option' })
  options!: Array<any>;
  ...
  override render() {
    ...
    const classes = {
      button: true,
      [`kd-split-btn--${baseTypeClass}${destructModifier}`]: true,
      [`kd-split-btn--${baseTypeClass}`]: !this.destructive,
      'kd-split-btn--large': this.size === SPLIT_BTN_SIZES.LARGE,
      'kd-split-btn--small': this.size === SPLIT_BTN_SIZES.SMALL,
      'kd-split-btn--medium': this.size === SPLIT_BTN_SIZES.MEDIUM,
      [`kd-split-btn--icon-${this.iconPosition}`]: !!this.iconPosition,
    };
    ...
    return html`
      ...
      >
        ...
        >
          <!-- Can put <kd-splitbutton-option> component here for each option -->
  ...
  private handleListBlur(e: any) {
    ...
    if (
      e.relatedTarget &&
      e.relatedTarget.localName !== 'kd-splitbutton-option'
    ) {
      this.open = false;
    }
  ...
  private _handleBlur(e: any): void {
    ...
    if (
      !relatedTarget ||
      (relatedTarget.localName !== 'kd-splitbutton-option' &&
        relatedTarget.localName !== 'kd-split-btn')
    ) {
      this.open = false;
    }
  }

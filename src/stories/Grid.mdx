...
.your-css-selector {
  @include grid.grid($align-left: false, $no-max: false, $no-pad: false);
}
...
Add spacing around the grid to prevent it from going full width by using a utility class or css variable on a container element.
